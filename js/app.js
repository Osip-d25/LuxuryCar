...
*/    var _suppressOverwrites, _reverting, _context, _globalTimeline, _win, _coreInitted, _doc, _coreReady, _lastRenderedFrame, _quickTween, _tickerActive, _config = {
        ...
        _self = {
            ...
            wake: function wake() {
                if (_coreReady) {
                    ...
                    _raf = "undefined" !== typeof requestAnimationFrame && requestAnimationFrame;
                    _id && _self.sleep();
    ...
    var Animation = function() {
        ...
        _proto.progress = function progress(value, suppressEvents) {
            return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(1 & this.iteration()) ? 1 - value : value) + _elapsedCycleDuration(this), suppressEvents) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
        };
        ...
        _proto.timeScale = function timeScale(value, suppressEvents) {
            ...
            this.totalTime(_clamp(-Math.abs(this._delay), this._tDur, tTime), false !== suppressEvents);
            _setEnd(this);
    ...
    var CSSPlugin_win, CSSPlugin_doc, _docElement, _pluginInitted, _tempDiv, _recentSetterPlugin, CSSPlugin_reverting, _supports3D, CSSPlugin_windowExists = function _windowExists() {
        ...
        if (property) {
            ...
            if (style.removeProperty) {
                ...
                if ("ms" === first2Chars || "webkit" === property.substr(0, 6)) property = "-" + property;
        ...
        if (cache && toPercent && cache.width && horizontal && cache.time === _ticker.time && !cache.uncache) return _round(curValue / cache.width * amount); else {
            if (toPercent && ("height" === property || "width" === property)) {
...
*/    var Observer_gsap, Observer_coreInitted, Observer_win, Observer_doc, _docEl, _body, _isTouch, _pointerType, ScrollTrigger, _root, _normalizer, _eventTypes, Observer_context, _getGSAP = function _getGSAP() {
        ...
        if (!Observer_coreInitted && Observer_gsap && "undefined" !== typeof document && document.body) {
            Observer_win = window;
...
*/
    var ScrollTrigger_gsap, ScrollTrigger_coreInitted, ScrollTrigger_win, ScrollTrigger_doc, ScrollTrigger_docEl, ScrollTrigger_body, ScrollTrigger_root, _resizeDelay, _toArray, ScrollTrigger_clamp, _time2, _syncInterval, _refreshing, _pointerIsDown, ScrollTrigger_transformProp, _i, _prevWidth, _prevHeight, _autoRefresh, _sort, ScrollTrigger_suppressOverwrites, _ignoreResize, ScrollTrigger_normalizer, _ignoreMobileResize, _baseScreenHeight, _baseScreenWidth, _fixIOSBug, ScrollTrigger_context, _scrollRestoration, _div100vh, _100vh, _isReverted, _clampingMax, _limitCallbacks, _rafID, _refreshingAll, _queueRefreshID, _primary, ScrollTrigger_startup = 1, ScrollTrigger_getTime = Date.now, _time1 = ScrollTrigger_getTime(), _lastScrollTime = 0, _enabled = 0, _parseClamp = function _parseClamp(value, type, self) {
        ...
        _triggers.forEach((function(t) {
            var max = _maxScroll(t.scroller, t._dir), endClamp = "max" === t.vars.end || t._endClamp && t.end > max, startClamp = t._startClamp && t.start >= max;
    ...
    var ScrollTrigger_ScrollTrigger = function() {
        ...
        _proto.init = function init(vars, animation) {
                ...
                if (pin && !(_clampingMax && self.end >= _maxScroll(scroller, direction))) {
                    ...
                    if (pinSpacing) {
                        ...
                        if (i) {
                            ...
                            "auto" !== spacer.style.flexBasis && (spacer.style.flexBasis = i + _px);
                        }
                        _setState(spacerState);
                        ...
                        i && "auto" !== spacer.style.flexBasis && (spacer.style.flexBasis = i + _px);
                    }
                    if (useFixedPosition) {
                        override = {
            ...
            self.update = function(reset, recordVelocity, forceFake) {
                ...
                if (anticipatePin && pin && !_refreshing && !ScrollTrigger_startup && _lastScrollTime) if (!clipped && start < scroll + (scroll - scroll2) / (ScrollTrigger_getTime() - _time2) * anticipatePin) clipped = 1e-4; else if (1 === clipped && end > scroll + (scroll - scroll2) / (ScrollTrigger_getTime() - _time2) * anticipatePin) clipped = .9999;
    ...
    const heroLoader = gsapWithCSS.timeline();
    heroLoader.set(".header__logo", {
        x: 100,
        opacity: 0
    }).set(".menu__item", {
        x: 100,
    ...
    function delayJs() {
        ...
        if (width1180.matches) {
            ...
            const mobileTlServices = gsapWithCSS.timeline({
                scrollTrigger: {
                    trigger: services,
                    start: "250 center",
                    end: "+=0",
        ...
        } else if (width319.matches) {
            const mobileSmallTlDetail = gsapWithCSS.timeline({
                scrollTrigger: {
                    trigger: detail,
                    start: "10 center",
                    end: "+=0",
            ...
            mobileSmallTlDetail.from(detail.querySelector(".detail__title"), {
                opacity: 0,
                yPercent: -40,
                duration: .5
            }, "-=0.1").from(detail.querySelector(".detail__text"), {
            ...
            const mobileSmallTlServices = gsapWithCSS.timeline({
                scrollTrigger: {
                    trigger: services,
                    start: "400 center",
                    end: "+=0",
            ...
            mobileSmallTlServices.fromTo(services.querySelectorAll(".services__info"), {
                x: 200,
                opacity: 0
            }, {
                x: 0,
            ...
            const mobileSmallTlLuxury = gsapWithCSS.timeline({
                scrollTrigger: {
                    trigger: luxury,
                    start: "400 center",
                    end: "+=0",
            ...
            mobileSmallTlLuxury.fromTo(luxury.querySelector(".luxury__title"), {
                opacity: 0,
                x: -15
            }, {
                opacity: 1,
